    ...
    def isAtTurningRTC(self, row, col):
        if (self.direction[1] == 1 or
            ...
            if self.location[2] % self.cellDim == 0:
                if row + 1 >= self.rows:
                    return self.board[int(row) - 1][int(col)] == 1
                elif row - 1 < 0:
                    return self.board[int(row) + 1][int(col)] == 1
                else:
                    return (self.board[int(row) + 1][int(col)] == 1 or
                            self.board[int(row) - 1][int(col)] == 1)
        return False
        ...
        if (self.direction[0] == 1 or
            ...
            if self.location[3] % self.cellDim == 0:
                if col + 1 >= self.cols:
                    ...
                    return (self.board[int(row)][int(col) + 1] == 1 or
                            self.board[int(row)][int(col) - 1] == 1)
        return False

    def moveEnemyRow(self, row, col):
        if self.direction[0] == 1:
            if row + 1 < int(self.rows) and self.board[int(row) + 1][int(col)] != 1:
                self.direction[0] = 0
            elif row + 1 >= self.rows:
                self.direction[0] = 0
        elif self.direction[0] == -1:
            if row - 1 > 0 and self.board[int(row) - 1][int(col)] != 1:
                self.direction[0] = 0
            elif row - 1 <= 0:
                self.direction[0] = 0
        elif self.direction[0] == 0:
            if row + 1 < self.rows and self.board[int(row) + 1][int(col)] == 1:
                self.direction[0] = 1
            elif row - 1 > 0 and self.board[int(row) - 1][int(col)] == 1:
                self.direction[0] = -1

    def moveEnemyCol(self, row, col):
        if self.direction[1] == 1:
            if col + 1 < self.cols and self.board[int(row)][int(col) + 1] != 1:
                self.direction[1] = 0
            elif col + 1 >= self.cols:
                self.direction[1] = 0
        elif self.direction[1] == -1:
            if col - 1 > 0 and self.board[int(row)][int(col) - 1] != 1:
                self.direction[1] = 0
            elif col - 1 <= 0:
                self.direction[1] = 0
        elif self.direction[1] == 0:
            if col + 1 < int(self.cols) and self.board[int(row)][int(col) + 1] == 1:
                self.direction[1] = 1
            elif col - 1 > 0 and self.board[int(row)][int(col) - 1] == 1:
                self.direction[1] = -1
